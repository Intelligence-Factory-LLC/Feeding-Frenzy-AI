...
{
	public class AutotaskHelper : IIntegrationProcessor
	{
		...
		public void OnCallCompleted(int iCallID)
		{
			this.AttachTranscriptionToTicket(iCallID);
		}
		...
		public void CreateTicket(int CompanyID, int CallID, DateTime DueDateTime)
		{
			...
			rowCall.ExternalKey = strTicketID;
			rowCall.ParentExternalKey = CompanyID.ToString();
		...
		public void CreateTicket(AutotaskTickets autotaskTickets, string strCompanyID, LeadsRow rowLead, CallsRow rowCall)
		{
			...
			rowCall.ExternalKey = strTicketID;
			rowCall.ParentExternalKey = strCompanyID;
			CallsRepository.UpdateCallData(rowCall);

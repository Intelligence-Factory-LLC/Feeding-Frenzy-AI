...
namespace FeedingFrenzy.Admin.Pages.tw
{
	...
	public class call_inModel : BaseModel
    {
		private async Task<IActionResult> HandleRequest()
		{
			...
			try
			{
				...
				Logs.DebugLog.WriteEvent("Twilio Call Request Query", SerializeRequestQueryAndForm());
				...
				SalesRepresentativesRow? rowSalesRepresentative = Leads.GetSalesRepresentativeByLeadByPhone(strCallerID);
				if (null != rowSalesRepresentative && !StringUtil.IsEmpty(rowSalesRepresentative.Phone) && rowSalesRepresentative.IsEnabled)
					strPhone = rowSalesRepresentative.Phone;
				...
				string strResponse = TwilioCalls.StartIncomingCall(startCallParams);
